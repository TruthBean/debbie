...
public class BeanFactoryHandler {
    ...
    public <T> T factoryBean(Class<T> type) {
        LOGGER.trace(() -> "factory bean with type " + type.getName());
        return factoryBean(null, type, true, true);
    }

    protected <T> T factoryBean(String serviceName, Class<T> type, boolean require, boolean throwException) {
        var beanInfo = getBeanInfo(serviceName, type, require, beanServiceInfoSet, throwException);
        if (!require && beanInfo == null)
            return null;
        assert beanInfo != null;
        return factoryBean(beanInfo);
    }

    public <T, K extends T> T factoryBean(DebbieBeanInfo<K> beanInfo) {
